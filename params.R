...
set_param<-function(){
  ...
  param$min.A=0.0               #proportion of population with reduced vulnerability (range: 1-0.1, 1 = no complexity)
  ...
  param$emerge=1100       #maximum refuge size in body mass (range explored = 1-1500) 
  param$min.A=0
  
  #Feeding preferences
  param$pref.pel=0.17           #Preference for "pelagic" carnivorous prey 
  param$pref.ben=0.5            #Preference for "benthic" invertebrate prey
  param$pref.herb=0.33          #Preference for herbivorous prey

  param$pref.det = 0
  param$pref.alg = 1
  
  #Fishing
  param$Fmort_pred=0.8            #Level of fishing mortality rate yr^(-1)
  param$Fmort_herb=0.1
  param$min.fishing.size=0.5      #Minimum log10 body size fished
...
#******NEW
  ...
  param$tmaxyears=100.0                           #Maximum number of years for model run
  ...
  param$ref.dst_10<-((param$dat_start_10-param$xmin)/param$dx)+1      # position in x of fish bigger than ~10cm (surveyable), this line did have 5 in place of 10 in dat_start_X
