#ARogers
#setwd('\Desktop - Copy\Natural Sciences Year 3\Term 2\NSC3002\Modelling\Alice Rogers Model\Model Code')
...
#Set refuges as per test location "Pasi_Gusung_North_nonreserve2" 
  # can comment out for future runs
...
#*The Captions for the 4 below graphs are working Here ****
...
legend(x = "topright", legend = c("Algae", "Herbivores"), col = c("seagreen", "green"), lty = 1)
...
#Detritus and invertebrates through time, ylim= c(0,300) is original
...
legend(275, 275, legend = c("Detritus", "Invertivores"), col = c("grey", "brown"), lty = 1)
...
#Plot equilibrium Biomass against Fishing Mortality Rate
  fishing_mortality_rate = c(0.1, 0.5, 1)
  #params$Fmort_pred 
  pred_biomass_varied_fishing = (output$Mod_pred_biomass[i])
  plot(pred_biomass_varied_fishing[[i]])~params$x, type = "l", xlim = c(), ylim = c(), main = "Predator Biomass", col = 'black')
  points(pred_biomass_varied_fishing, type = "l", col = "red")
...
#Plot outputs for checking
  ...
  plot(Inv_growth_data[[i]]~params$x, type = "l", xlim = c(-1.5, 5), ylim = c(0,3), main = "Invertebrate growth"
       , col = "brown")
  legend(x="topleft", legend = c("Predator growth", "Herbivore growth", "Invertebrate growth"), col = c("red", "green", "brown"), lty = 1)
  ...
  plot(Inv_mort_data[[i]]~params$x, type = "l", xlim = c(-1.5, 5), ylim = c(0,20), main = "Invertebrate 
       mortality", col = "brown")
  legend(x="topleft", legend = c("Predator mortality", "Herbivore mortality", "Invertebrate mortality"), col = c("red", "green", "brown"), lty = 1)
  
  plot(Refuge_data[[i]]~params$x, type = "l", xlim = c(-1.5, 5), ylim = c(0,1), main = "Refuge"
       , col = "black")
  text(4,0.8, count[i])
  legend(0, 40, legend = c("Predator mortality", "Herbivore mortality", "Invertebrate mortality"), col = c("red", "green", "brown"), lty = 1)
