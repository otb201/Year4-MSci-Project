...
#-------------------------
    ...
    #To match data of fish > 3.5cm
    #Pred_dat_bio_3.5<-colSums(U[ref.dst_3.5:ref.den,]*dx*10^x[ref.dst_3.5:ref.den]) #need to look at line 156 of params.R
    #Herb_dat_bio_3.5<-colSums(H[ref.dst_3.5:ref.den,]*dx*10^x[ref.dst_3.5:ref.den])
    ...
    #Proportional refuges
     ...
     return(list(Body.size = x, Preds=U[,N-1], P_grth=GG.u[, N-1], P_mrt=PM.u[, N-1],
                  ...
                  Pred_dat_gm_10 = Pred_dat_bio_10[length(Pred_dat_bio_10)],
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------
#================================
# Test carbon (BIOMASS) conservation:
#================================
# biomass in:
#phytoplankton <- array(0, c(length(x), 1))
#phytoplankton[1:(ref)]  <- preds[1:(ref),1]  

#small_invs <- array(0, c(length(x), 1))
#small_invs[refinv:(ref.det)]  <- dets[refinv:(ref.det),1]   

#feeding <- as.vector((vulnerability[,i]*f.pel_const)*(phytoplankton[,1])%*%(gphi)) + as.vector(f.ben_const * (small_invs[,1])%*%(gphi))
#total_feeding <- sum(feeding[(ref+1):end] * preds[(ref+1):end,i]*log10dx*10^x[(ref+1):end])

#biomass_in <- input.a.k + alr.t + alr.p + total_feeding + sum((intrinsic_mortality_mass) * phytoplankton[,1] + (intrinsic_mortality_mass) * small_invs[,1]) * log10dx
#print(biomass_in)

# biomass out
#biomass_out <-  sum((1-K.pred - def.high)*PM.pred[(ref+1):end]* preds[(ref+1):end,i]*log10dx*10^x[(ref+1):end]) +     #PM.pred is mortality due to predation of predators, preds is number of predators
  #sum(((1-K.a) * f.alg.k + (1-K.a) * f.alg.t + (1-K.d) * f.det_H)*herb.kelp[,i]*log10dx*10^x) +                       #f.alg is feeding on algae
  #sum((1-K.h - def.low) * PM.h.k[(ref+1):end]*herb.kelp[(ref+1):end,i]*log10dx*10^x[(ref+1):end]) +                   #f.det is feeding on detritus
  #sum(((1-K.p) * f.alg.p)*herb.plank[,i]*log10dx*10^x) + 
  #sum((1-K.h - def.low) * PM.h.p[(ref+1):end]*herb.plank[(ref+1):end,i]*log10dx*10^x[(ref+1):end]) +
  #sum(((1-K.d) * f.det)*dets[,i]*log10dx*10^x) + 
  #sum((1-K.v - def.low) * PM.det[(ref.det+1):(inv_end)]*dets[(ref.det+1):(inv_end),i]*log10dx*10^x[(ref.det+1):(inv_end)])
...
plotbiomass <- function(modeloutput, params){
  ...
  with(params, {
    ...
    plot(modeloutput$p_bio_time, type = "l", col = "red", ylim = c(0,max(modeloutput$p_bio_time + 30)), main = "Biomass Growth")
    points(modeloutput$h_bio_time, type = "l", col = "green")
    points(modeloutput$i_bio_time, type = "l", col = "brown")
    legend(x= "topright", legend = c("Predator", "Herbivore", "Invertebrate"), col = c("red", "green", "brown"), lty = 1)
